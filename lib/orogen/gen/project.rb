...
module OroGen
    module Gen
        module RTT_CPP
            def self.extended_states=(value)
                @extended_states = value
            end

            def self.extended_states_enabled?
                @extended_states
            end
            @extended_states = true

            def self.define_default_deployments=(value)
                @define_default_deployments = value
            end

            def self.define_default_deployments_enabled?
            ...
            class Project
                ...
                def generate_build_system # :nodoc:
                    ...
                    Dir.glob File.join(Generation.template_path("config"), "*") do |path|
                        ...
                        unless CMAKE_GENERATED_CONFIG.include?(basename)
                ...
                def tasklib_dependencies
                    ...
                    result.to_set.to_a.sort_by(&:var_name)
                end
                ...
                def typekit(create = nil, &block)
                    ...
                    if create && !@typekit
                        ...
                        typekit.include_dirs |=
                            used_task_libraries.map(&:include_dirs)
                                               .flatten.to_set
